...
func normalPlan(ctx context.Context, opts *PlanOpts, prevRoundState *states.State, configInst *eval.ConfigInstance, providers plugins.Providers) (*plans.Plan, tfdiags.Diagnostics) {
	...
	planCtx.rootOutput.PrevValues = prevRoundState.EnsureModule(addrs.RootModuleInstance).OutputValues
	planCtx.rootOutput.Values, planCtx.rootOutput.ResourceDependencies = planGlue.oracle.RootOutputs(ctx)
